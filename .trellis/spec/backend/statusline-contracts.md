...
- The Codex item catalog must use the current official ids: `model`, `model-with-reasoning`, `reasoning`, `current-dir`, `project-name`, `hostname`, `git-branch`, `pull-request-number`, `branch-changes`, `run-state`, `permissions`, `approval-mode`, `context-remaining`, `context-used`, `five-hour-limit`, `weekly-limit`, `codex-version`, `context-window-size`, `used-tokens`, `total-input-tokens`, `total-output-tokens`, `thread-credits`, `estimated-thread-cost`, `thread-id`, `fast-mode`, `raw-output`, `thread-title`, `workspace-headline` and `task-progress`.
- Codex legacy aliases `model-name`, `project`/`project-root`, `status`, `approval`, `context-usage` and `session-id` are accepted on read/save and normalized to their official ids. Unknown ids supplied for save return `codex_statusline_unknown_item` instead of writing invalid configuration; `thread-name` is not an official or supported id and must be corrected to `thread-title`.
- When adopting an external Codex configuration or loading an existing profile, unknown item ids are preserved in the stored snapshot so the editor can display and remove them; applying or saving that snapshot still performs strict item validation.
