...
import {
  ...
  type BackupSnapshotV3,
} from "../../../sync/api/syncStore";
import { Check, Cloud, Download, Folder, Trash2, Upload } from "../../../../shared/ui/icons";
...
export function SyncSettingsPage() {
  ...
  useEffect(() => {
    ...
    setUrl(store.webdavUrl);
    setUsername(store.webdavUsername);
    setPassword(store.getSessionPassword());
    setDeviceName(store.deviceName);
    setRemoteDir(store.remoteDir);
  }, [
    store.loaded,
    store.webdavUrl,
  ...
  const domainOptions = useMemo(
    () => DOMAINS.map((domain) => ({ domain, label: t(`settings.sync.backup.domain.${domain}`) })),
    [t],
  );
  ...
  const saveConnection = async (test: boolean) => {
    ...
    try {
      ...
      toast.success(
        test ? t("settings.sync.backup.connectionPassed") : t("settings.sync.backup.saved"),
      );
    } catch (error) {
  ...
  const saveIdentity = async () => {
    try {
      await store.setDeviceName(deviceName);
      await store.setRemoteDir(remoteDir.trim());
  ...
  const createBackup = async () => {
    try {
      ...
      if (error instanceof Error && error.message === "backup_queued")
        toast.warning(t("settings.sync.backup.queued"));
      else
        toast.error(t("settings.sync.backup.operationFailed"), {
  ...
  const pickLocalDir = async () => {
    const result = await openDialog({
      directory: true,
      multiple: false,
      title: t("settings.sync.backup.chooseDirectory"),
    });
  ...
  const restoreCloud = async () => {
    ...
    try {
      ...
      toast.success(t("settings.sync.backup.restored"));
  ...
  const domainSelector = (
    <Stack gap="xs">
      <Text size="sm" fw={600}>
        {t("settings.sync.backup.restoreDomains")}
      </Text>
      <Group gap="md">
      ...
      {selectedDomains.includes("workspace") && (
        <Text size="xs" c="var(--on-surface-variant)">
          {t("settings.sync.backup.appearanceOverwriteNote")}
        </Text>
      )}
    </Stack>
  );
  const previewSummary = (preview: BackupSnapshotV3) =>
    t("settings.sync.backup.previewSummary", {
      projects: preview.data.workspace.projects.length,
      groups: preview.data.workspace.groups.length,
      worktrees: preview.data.workspace.worktrees.length,
      templates: preview.data.workspace.commandTemplates.length,
      settings: Object.keys(preview.data.preferences).length,
      prices: preview.data.modelPrices.length,
      targets: preview.data.notifications.targets.length,
      nativeProviders: preview.data.nativeProviders?.length ?? 0,
      nativeKeys:
        preview.data.nativeProviders?.filter((provider) => provider.keyReentryRequired).length ?? 0,
    });
    ...
    <Stack gap="md">
      <Card className="border border-yellow-500/30 bg-yellow-500/10" p="md" radius="lg">
        <Text size="sm" fw={600} c="yellow">
          {t("settings.sync.backup.plaintextTitle")}
        </Text>
        <Text mt={4} size="sm" c="var(--on-surface-variant)">
          {t("settings.sync.backup.plaintextWarning")}
        </Text>
      </Card>
      ...
      <section className="ui-surface-card rounded-2xl border border-border p-4">
        <Stack gap="md">
          ...
          <Select<BackupMode>
            value={store.backupMode}
            allowDeselect={false}
            data={[
              { value: "cloud", label: t("settings.sync.backup.modeCloud") },
              { value: "local", label: t("settings.sync.backup.modeLocal") },
            ]}
            onChange={(value) => value && void store.setBackupMode(value)}
          />
          <Switch
            checked={store.autoBackupOnClose}
            onChange={(event) => void store.setAutoBackupOnClose(event.currentTarget.checked)}
        ...
        <>
          <section className="ui-surface-card rounded-2xl border border-border p-4">
            <Stack gap="md">
              ...
              <Group grow>
                <TextInput
                  label={t("settings.sync.backup.username")}
                  value={username}
                  onChange={(event) => setUsername(event.currentTarget.value)}
                />
                <PasswordInput
                  label={t("settings.sync.backup.password")}
                  value={password}
              ...
              <Group grow>
                <TextInput
                  label={t("settings.sync.backup.deviceName")}
                  value={deviceName}
                  onChange={(event) => setDeviceName(event.currentTarget.value)}
                />
                <TextInput
                  label={t("settings.sync.backup.remoteDirectory")}
                  value={remoteDir}
                  onChange={(event) => setRemoteDir(event.currentTarget.value)}
                  placeholder="cli-manager"
                />
              </Group>
              <Group>
                <Button size="xs" onClick={() => void saveConnection(true)} loading={testing}>
                  {t("settings.sync.backup.testConnection")}
                </Button>
                <Button size="xs" variant="default" onClick={() => void saveConnection(false)}>
                  {t("settings.sync.backup.saveConnection")}
                </Button>
                <Button size="xs" variant="default" onClick={() => void saveIdentity()}>
                  {t("settings.sync.backup.saveIdentity")}
                </Button>
                {store.hasPassword && (
                  <Group gap={4} c="var(--success)">
                    <Check size={14} />
                    <Text size="xs">{t("settings.sync.backup.configured")}</Text>
                  </Group>
                )}
              </Group>
            </Stack>
              ...
              <Group justify="space-between">
                ...
                <Group gap="xs">
                  <Button
                    ...
                    disabled={!store.hasPassword || busy}
                    onClick={() => void createBackup()}
                  >
                    {t("settings.sync.backup.createNow")}
                  </Button>
                  <Button
                    size="xs"
                    variant="default"
                    ...
                    disabled={!store.hasPassword || busy}
                    onClick={() => void store.listBackups()}
                  >
                    {t("settings.sync.backup.refresh")}
                  </Button>
                  <Button
                    size="xs"
                    variant="subtle"
                  ...
                  >
                    {t("settings.sync.backup.importLegacyCloud")}
                  </Button>
                </Group>
              </Group>
              {store.snapshots.length === 0 ? (
                <Text size="sm" c="var(--on-surface-variant)">
                  {t("settings.sync.backup.empty")}
                </Text>
              ) : (
                store.snapshots.map((snapshot) => (
                  <Card
                    key={snapshot.remotePath}
                  ...
                  >
                    <Group justify="space-between" wrap="nowrap">
                      <Stack gap={2}>
                        <Text size="sm" fw={600}>
                          {snapshot.manifest.deviceName}
                        </Text>
                        <Text size="xs" c="var(--on-surface-variant)">
                          {formatTime(snapshot.manifest.createdAt)} · {snapshot.manifest.appVersion}
                        </Text>
                      </Stack>
                      <Group gap="xs">
                        <Button
                          ...
                          onClick={() => void openRestorePreview(snapshot)}
                        >
                          {t("settings.sync.backup.restore")}
                        </Button>
                        <Button
                          size="xs"
                          variant="subtle"
          ...
          <Stack gap="md">
            ...
            <Group wrap="nowrap">
              <TextInput
                className="flex-1"
                readOnly
                value={store.localBackupDir}
                placeholder={t("settings.sync.backup.noDirectory")}
              />
              <Button
                variant="default"
              ...
              >
                {t("settings.sync.backup.chooseDirectory")}
              </Button>
            </Group>
            <Group>
              <Button
                leftSection={<Upload size={14} />}
                disabled={!store.localBackupDir || busy}
                onClick={() => void createBackup()}
              >
                {t("settings.sync.backup.exportZip")}
              </Button>
              <Button
                variant="default"
                leftSection={<Download size={14} />}
              ...
              >
                {t("settings.sync.backup.importZip")}
              </Button>
            </Group>
          </Stack>
        </section>
      ...
      <Button
        ...
        onClick={() =>
          void store
            .undoLastRestore()
            .then(() => toast.success(t("settings.sync.backup.undoDone")))
            .catch((error) =>
              toast.error(t("settings.sync.backup.operationFailed"), {
                description: errorMessage(error),
              }),
      ...
      <Modal
        opened={Boolean(selectedSnapshot)}
        onClose={() => {
          setSelectedSnapshot(null);
          setSelectedPreview(null);
        }}
        title={t("settings.sync.backup.confirmRestore")}
        centered
      >
        <Stack gap="md">
          {selectedPreview && (
            <Card className="bg-surface-container-low" p="sm" radius="lg">
              <Text size="sm">{previewSummary(selectedPreview)}</Text>
              {providerReentryNote(selectedPreview) && (
          ...
          {domainSelector}
          <Text size="sm" c="var(--on-surface-variant)">
            {t("settings.sync.backup.restoreSafetyNote")}
          </Text>
          <Group justify="flex-end">
            ...
            >
              {t("settings.sync.backup.confirm")}
            </Button>
          </Group>
        </Stack>
      </Modal>
      <Modal
        opened={Boolean(localImportPath)}
        onClose={() => {
          setLocalImportPath(null);
          setLocalPreview(null);
        }}
        title={t("settings.sync.backup.confirmImport")}
        centered
      >
        <Stack gap="md">
          {localPreview && (
            <Card className="bg-surface-container-low" p="sm" radius="lg">
              <Text size="sm">{previewSummary(localPreview)}</Text>
              {providerReentryNote(localPreview) && (
                <Text size="xs" c="dimmed" mt={4}>
                  {providerReentryNote(localPreview)}
                </Text>
              )}
      ...
      <Modal
        opened={Boolean(deleteSnapshot)}
        onClose={() => setDeleteSnapshot(null)}
        title={t("settings.sync.backup.confirmDelete")}
        centered
      >
        <Stack gap="md">
          <Text size="sm" c="var(--on-surface-variant)">
            {t("settings.sync.backup.deleteWarning")}
          </Text>
          <Group justify="flex-end">
            <Button variant="default" onClick={() => setDeleteSnapshot(null)}>
              {t("common.cancel")}
            </Button>
            <Button
              color="red"
              onClick={() => {
                ...
                if (snapshot)
                  void store
                    .deleteBackup(snapshot.remotePath)
                    .catch((error) =>
                      toast.error(t("settings.sync.backup.operationFailed"), {
                        description: errorMessage(error),
                      }),
      ...
      <Modal
        opened={legacyImportOpen}
        onClose={() => setLegacyImportOpen(false)}
        title={t("settings.sync.backup.confirmLegacyImport")}
        centered
      >
        <Stack gap="md">
          {domainSelector}
          <Text size="sm" c="var(--on-surface-variant)">
            {t("settings.sync.backup.legacyImportNote")}
          </Text>
          <Group justify="flex-end">
            <Button variant="default" onClick={() => setLegacyImportOpen(false)}>
              {t("common.cancel")}
            </Button>
            <Button
              color="red"
              ...
              onClick={() => {
                ...
                void store
                  .importLegacyCloud(selectedDomains)
                  .then(() => toast.success(t("settings.sync.backup.restored")))
                  .catch((error) =>
                    toast.error(t("settings.sync.backup.operationFailed"), {
                      description: errorMessage(error),
                    }),
