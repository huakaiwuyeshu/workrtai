...
import { useSettingsStore, type StatuslineEditorSource } from "../../../../shared/preferences/settingsStore";
import { SettingsListItem } from "../SettingsListItem";
import { StatuslineProfileBar } from "../StatuslineProfileBar";
import {
  useStatuslineProfiles,
  type StatuslineImportAnalysis,
  type StatuslineImportDecision,
  type StatuslineProfileState,
} from "../../lib/statuslineProfiles";
import { useAppPrompt } from "../../../../shared/ui/useAppPrompt";
import { useAppConfirm } from "../../../../shared/ui/useAppConfirm";

const COLOR_DEFINITIONS = [
  ["", "默认 (Default)", "transparent"],
  ["black", "黑色 (Black)", "#000000"],
  ["red", "红色 (Red)", "#cc0000"],
  ["green", "绿色 (Green)", "#4e9a06"],
  ["yellow", "黄色 (Yellow)", "#c4a000"],
  ["blue", "蓝色 (Blue)", "#3465a4"],
  ["magenta", "品红 (Magenta)", "#75507b"],
  ["cyan", "青色 (Cyan)", "#06989a"],
  ["white", "白色 (White)", "#d3d7cf"],
  ["brightBlack", "亮黑色 (Bright Black)", "#555753"],
  ["brightRed", "亮红色 (Bright Red)", "#ef2929"],
  ["brightGreen", "亮绿色 (Bright Green)", "#8ae234"],
  ["brightYellow", "亮黄色 (Bright Yellow)", "#fce94f"],
  ["brightBlue", "亮蓝色 (Bright Blue)", "#729fcf"],
  ["brightMagenta", "亮品红 (Bright Magenta)", "#ad7fa8"],
  ["brightCyan", "亮青色 (Bright Cyan)", "#34e2e2"],
  ["brightWhite", "亮白色 (Bright White)", "#eeeeec"],
] as const;
const COLOR_OPTIONS = COLOR_DEFINITIONS.map(([value, label]) => ({ value, label }));
const COLOR_SWATCHES = new Map<string, string>(
  COLOR_DEFINITIONS.map(([value, , color]) => [value, color]),
);
  ...
  label:
    value === "custom"
      ...
      : value
          ...
          .join(" ")
          .replace("Onedark", "One Dark")
          .replace("Tokyonight", "Tokyo Night"),
}));
const POWERLINE_SEPARATORS = [
  { value: "\ue0b0", label: " 右三角 (Right Triangle)" },
  { value: "\ue0b2", label: " 左三角 (Left Triangle)" },
  { value: "\ue0b4", label: " 右圆弧 (Right Round)" },
  { value: "\ue0b6", label: " 左圆弧 (Left Round)" },
];
const POWERLINE_START_CAPS = [
  { value: "", label: "无 (None)" },
  { value: "\ue0b2", label: " 三角 (Triangle)" },
  { value: "\ue0b6", label: " 圆弧 (Round)" },
  { value: "\ue0ba", label: " 下三角 (Lower Triangle)" },
  { value: "\ue0be", label: " 斜线 (Diagonal)" },
];
const POWERLINE_END_CAPS = [
  { value: "", label: "无 (None)" },
  { value: "\ue0b0", label: " 三角 (Triangle)" },
  { value: "\ue0b4", label: " 圆弧 (Round)" },
  { value: "\ue0b8", label: " 下三角 (Lower Triangle)" },
...
interface PowerlineFontStatus {
  installed: boolean;
  checkedSymbol: string;
  matchedFont: string | null;
}
interface PowerlineFontInstallResult {
  success: boolean;
  message: string;
...
const renderColorOption = ({ option }: { option: { value: string; label: string } }) => (
  <Group gap="xs" wrap="nowrap">
    <ColorSwatch
      size={16}
      color={COLOR_SWATCHES.get(option.value) ?? "transparent"}
      withShadow={option.value !== ""}
    />
...
function ClaudeStatuslineEditor({
  ...
  const normalizedTerminalFontFamily = useMemo(
  ...
  useEffect(() => {
    refresh().catch((error) =>
      toast.error(t("settings.statusline.loadFailed"), { description: errorMessage(error) }),
    );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      invoke<string>("statusline_render_preview", {
        settings,
        payload: STATUSLINE_PREVIEW_PAYLOAD,
        language,
      })
        .then(setPreview)
        .catch((error) =>
          setPreview(`${t("settings.statusline.previewFailed")}: ${errorMessage(error)}`),
        );
    }, 120);
  ...
  const filteredCatalog = useMemo(() => {
    ...
    return catalog.filter((entry) =>
      `${entry.widgetType} ${entry.zhName} ${entry.enName} ${entry.category}`
        .toLowerCase()
        .includes(query),
    );
  }, [catalog, searchValue]);
  ...
  useEffect(() => {
    setOpenedCategories(
      searchValue.trim() ? groupedCatalog.map(([category]) => category) : ["core"],
    );
  ...
  const updateWidget = (patch: Partial<StatuslineWidget>) => {
    ...
    updateSettings((current) => ({
      ...
      lines: current.lines.map((line, lineIndex) =>
        lineIndex === selected.line
          ? line.map((widget, index) =>
              index === selected.index ? { ...widget, ...patch } : widget,
            )
          : line,
      ),
    }));
  ...
  const removeWidget = (id: string) => {
    updateSettings((current) => ({
      ...current,
      lines: current.lines.map((line) => line.filter((widget) => widget.id !== id)),
    }));
  ...
  const removeSelected = () => {
    ...
    updateSettings((current) => ({
      ...current,
      lines: current.lines.map((line, lineIndex) =>
        lineIndex === selected.line ? line.filter((_, index) => index !== selected.index) : line,
      ),
    }));
    setSelectedId(null);
  };
  ...
  const handleWidgetDragStart = ({ active }: DragStartEvent) => {
    const widgetType =
      active.data.current?.source === "catalog" ? active.data.current.widgetType : null;
  ...
  const handleWidgetDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    const catalogType =
      ...
      typeof active.data.current.widgetType === "string"
    ...
    for (let lineIndex = 0; lineIndex < settings.lines.length; lineIndex += 1) {
      ...
      if (over.id === `statusline-line-${lineIndex}`)
        to = { line: lineIndex, index: settings.lines[lineIndex].length };
    }
    if (!to) return;
    if (catalogType) {
  ...
  const moveSelectedToLine = (targetLine: number) => {
    ...
    moveWidget(
      { line: selected.line, index: selected.index },
      { line: targetLine, index: settings.lines[targetLine].length },
    );
    setActiveLineIndex(targetLine);
  };
  ...
  const toggleInstall = async () => {
    ...
    try {
      ...
      setStatus(next);
      toast.success(
        t(status?.installed ? "settings.statusline.uninstalled" : "settings.statusline.installed"),
      );
    } catch (error) {
  ...
  const installFonts = async () => {
    ...
    try {
      ...
      toast.success(
        t("settings.statusline.powerlineFontInstalled").replace(
          "{count}",
          String(result.installedCount),
        ),
      );
    } catch (error) {
      toast.error(t("settings.statusline.powerlineFontInstallFailed"), {
  ...
  const setPowerlineEnabled = (enabled: boolean) => {
    updateSettings((current) => ({
      ...
      lines: enabled
        ? current.lines.map((line) => line.filter((item) => item.type !== "separator"))
        : current.lines,
      powerline: {
        ...current.powerline,
        enabled,
        theme:
          enabled && (!current.powerline.theme || current.powerline.theme === "custom")
            ? "nord-aurora"
            : current.powerline.theme,
      },
    }));
  };

  const draggingCatalogEntry = draggingCatalogType
    ? catalog.find((entry) => entry.widgetType === draggingCatalogType)
    : null;
  ...
  if (!settings)
    return <Text c="var(--on-surface-variant)">{t("settings.statusline.loading")}</Text>;
    ...
    <Stack gap="md">
      <Card className="border border-border bg-surface-container-low" radius="lg" p="md">
        <StatuslineProfileBar
          ...
          onRename={async (profileId, name) =>
            applyProfileState(await profiles.rename(profileId, name))
          }
      ...
      <Card className="border border-border bg-surface-container-low" radius="lg" p="md">
        <Group justify="space-between" align="flex-start">
          <Box>
            <Group gap="xs">
              <Text fw={600}>{t("settings.statusline.runtime")}</Text>
              <Badge color={status?.installed ? "green" : "gray"}>
                {t(
                  status?.installed
                    ? "settings.statusline.enabled"
                    : "settings.statusline.disabled",
                )}
              </Badge>
            </Group>
            <Text size="xs" c="var(--on-surface-variant)" mt={4}>
              {status?.claudeSettingsPath}
            </Text>
          </Box>
          <Group gap="xs">
            {dirty && (
              <Badge color="yellow" variant="light">
                {t("settings.statusline.unsaved")}
              </Badge>
            )}
            {status?.legacySettingsAvailable && (
              <Button
                variant="light"
            ...
            >
              {t("settings.statusline.refresh")}
            </Button>
            <Button
              color={status?.installed ? "red" : "cliPrimary"}
              onClick={toggleInstall}
              loading={working}
            >
              {t(
                status?.installed ? "settings.statusline.uninstall" : "settings.statusline.install",
              )}
            </Button>
          </Group>
        </Group>
      ...
      <DndContext
        ...
        onDragCancel={() => setDraggingCatalogType(null)}
      >
        <Box className="grid gap-4 2xl:grid-cols-[320px_minmax(360px,1fr)_300px] xl:grid-cols-[300px_minmax(340px,1fr)]">
          <Card className="border border-border bg-surface-container-low" radius="lg" p="md">
            <Group justify="space-between" mb="sm">
              ...
              <Badge variant="light" color="cliPrimary">
                {t("settings.statusline.componentCount").replace("{count}", String(catalog.length))}
              </Badge>
            </Group>
            <ScrollArea.Autosize mah={540} type="auto" offsetScrollbars scrollbarSize={8}>
              {groupedCatalog.length > 0 ? (
                          ...
                          {entries.map((entry) => {
                            ...
                            return (
                              <CatalogWidgetItem
                                ...
                                ariaLabel={t("settings.statusline.addComponent").replace(
                                  "{name}",
                                  title,
                                )}
                              />
                ...
                <Text size="sm" c="var(--on-surface-variant)">
                  {t("settings.statusline.catalogEmpty")}
                </Text>
              )}
            </ScrollArea.Autosize>
          </Card>
          ...
          <Stack gap="md">
            <Card className="border border-border bg-surface-container-low" radius="lg" p="md">
              <Group justify="space-between">
                <Text fw={600}>{t("settings.statusline.layout")}</Text>
                <Button
              ...
              <Stack gap="sm" mt="sm">
                {settings.lines.map((line, lineIndex) => (
                  <StatuslineLayoutLine
                    key={lineIndex}
                    lineIndex={lineIndex}
                    itemIds={line.map((item) => item.id)}
                    active={activeLineIndex === lineIndex}
                    onClick={() => {
                      setActiveLineIndex(lineIndex);
                      setSelectedId(null);
                    }}
                  >
                    <Text size="xs" c="var(--on-surface-variant)" mb={6}>
                      {t("settings.statusline.line").replace("{line}", String(lineIndex + 1))}
                    </Text>
                    <Group gap={6} align="stretch">
                      {line.map((item) => {
                        const entry = catalog.find(
                          (candidate) => candidate.widgetType === item.type,
                        );
                        const label = entry
                          ? pickByLanguage(language, entry.zhName, entry.enName)
                          : item.type;
                        return (
                          <SortableWidgetChip
                            key={item.id}
                            ...
                            removeLabel={t("settings.statusline.removeComponent").replace(
                              "{name}",
                              label,
                            )}
                            onSelect={() => {
                      ...
                      {line.length === 0 && (
                        <Text size="xs" c="var(--on-surface-variant)">
                          {t("settings.statusline.dropHere")}
                        </Text>
                      )}
                    </Group>
                  </StatuslineLayoutLine>
          ...
          <Card
            className="border border-border bg-surface-container-low 2xl:col-auto xl:col-span-2"
            radius="lg"
            p="md"
          >
            <Text fw={600} mb="sm">
              {t("settings.statusline.properties")}
            </Text>
            {selected ? (
              <Stack gap="sm">
                <Text size="sm" fw={500}>
                ...
                <SegmentedControl
                  ...
                  data={settings.lines.map((_, index) => ({
                    value: String(index),
                    label: t("settings.statusline.line").replace("{line}", String(index + 1)),
                  }))}
                  aria-label={t("settings.statusline.moveToLine")}
                />
                <Select
                  label={t("settings.statusline.foreground")}
                  data={COLOR_OPTIONS}
                  value={selected.widget.color ?? ""}
                  onChange={(value) => updateWidget({ color: value || undefined })}
                  renderOption={renderColorOption}
                />
                <Select
                  label={t("settings.statusline.background")}
                  data={COLOR_OPTIONS}
                  value={selected.widget.backgroundColor ?? ""}
                  onChange={(value) => updateWidget({ backgroundColor: value || undefined })}
                  renderOption={renderColorOption}
                />
                <Checkbox
                  label={t("settings.statusline.bold")}
                  checked={selected.widget.bold ?? false}
                  onChange={(event) => updateWidget({ bold: event.currentTarget.checked })}
                />
                {selected.widget.type === "custom-text" && (
                  <TextInput
                    label={t("settings.statusline.text")}
                    value={selected.widget.customText ?? ""}
                    onChange={(event) => updateWidget({ customText: event.currentTarget.value })}
                  />
                )}
                {selected.widget.type === "custom-symbol" && (
                  <TextInput
                    label={t("settings.statusline.symbol")}
                    value={selected.widget.customSymbol ?? ""}
                    onChange={(event) => updateWidget({ customSymbol: event.currentTarget.value })}
                  />
                )}
                {selected.widget.type === "custom-command" && (
                  <>
                    <TextInput
                      label={t("settings.statusline.command")}
                      value={selected.widget.commandPath ?? ""}
                      onChange={(event) => updateWidget({ commandPath: event.currentTarget.value })}
                    />
                    <NumberInput
                      label={t("settings.statusline.timeout")}
                      min={100}
                      max={30000}
                      value={selected.widget.timeout ?? 2000}
                      onChange={(value) => updateWidget({ timeout: Number(value) || 2000 })}
                    />
                  </>
                )}
                <Button
              ...
              <Stack gap="md">
                <Text size="sm" c="var(--on-surface-variant)">
                  {t("settings.statusline.selectWidget")}
                </Text>
                <Text fw={600}>{t("settings.statusline.powerlineSettings")}</Text>
                <Group justify="space-between">
                  <Text size="sm">{t("settings.statusline.powerlineFontStatus")}</Text>
                  <Group gap="xs">
                    <Badge color={fontStatus?.installed ? "green" : "yellow"}>
                      {t(
                        fontStatus?.installed
                          ? "settings.statusline.powerlineFontInstalledStatus"
                    ...
                    {!fontStatus?.installed && (
                      <Button
                        size="xs"
                        variant="light"
                        loading={installingFonts}
                        onClick={() => void installFonts()}
                      >
                        {t("settings.statusline.powerlineFontInstall")}
                      </Button>
                    )}
                  </Group>
                </Group>
                {fontStatus?.installed && (
                  <Text size="xs" c="var(--on-surface-variant)">
                    {t("settings.statusline.powerlineFontActivateHint")}
                  </Text>
                )}
                {fontStatus?.matchedFont && (
                  <Text size="xs" c="var(--on-surface-variant)">
                    {fontStatus.matchedFont}
                  </Text>
                )}
                <Switch
                  label={t("settings.statusline.powerline")}
                  checked={settings.powerline.enabled}
                  onChange={(event) => setPowerlineEnabled(event.currentTarget.checked)}
                />
                <Switch
                  label={t("settings.statusline.powerlineAutoAlign")}
                  checked={settings.powerline.autoAlign}
                  disabled={!settings.powerline.enabled}
                  onChange={(event) =>
                    updateSettings((current) => ({
                      ...current,
                      powerline: { ...current.powerline, autoAlign: event.currentTarget.checked },
                    }))
                  }
                />
                <Switch
                  label={t("settings.statusline.powerlineContinueTheme")}
                  checked={settings.powerline.continueThemeAcrossLines}
                  disabled={!settings.powerline.enabled}
                  onChange={(event) =>
                    updateSettings((current) => ({
                      ...current,
                      powerline: {
                        ...current.powerline,
                        continueThemeAcrossLines: event.currentTarget.checked,
                      },
                    }))
                  }
                />
                <Select
                  label={t("settings.statusline.powerlineSeparator")}
                  data={POWERLINE_SEPARATORS}
                  disabled={!settings.powerline.enabled}
                  value={settings.powerline.separators[0] ?? "\ue0b0"}
                  classNames={POWERLINE_SELECT_CLASS_NAMES}
                  renderOption={({ option }) => (
                    <Text size="sm" ff={normalizedTerminalFontFamily}>
                      {option.label}
                    </Text>
                  )}
                  onChange={(value) =>
                ...
                />
                <Select
                  label={t("settings.statusline.powerlineStartCap")}
                  data={POWERLINE_START_CAPS}
                  disabled={!settings.powerline.enabled}
                  value={settings.powerline.startCaps[0] ?? ""}
                  classNames={POWERLINE_SELECT_CLASS_NAMES}
                  renderOption={({ option }) => (
                    <Text size="sm" ff={normalizedTerminalFontFamily}>
                      {option.label}
                  ...
                  onChange={(value) =>
                    updateSettings((current) => ({
                      ...current,
                      powerline: { ...current.powerline, startCaps: value ? [value] : [] },
                    }))
                  }
                />
                <Select
                  label={t("settings.statusline.powerlineEndCap")}
                  data={POWERLINE_END_CAPS}
                  disabled={!settings.powerline.enabled}
                  value={settings.powerline.endCaps[0] ?? ""}
                  classNames={POWERLINE_SELECT_CLASS_NAMES}
                  renderOption={({ option }) => (
                    <Text size="sm" ff={normalizedTerminalFontFamily}>
                      {option.label}
                  ...
                  onChange={(value) =>
                    updateSettings((current) => ({
                      ...current,
                      powerline: { ...current.powerline, endCaps: value ? [value] : [] },
                    }))
                  }
                />
                <Select
                  label={t("settings.statusline.powerlineTheme")}
                  data={POWERLINE_THEMES}
                  disabled={!settings.powerline.enabled}
                  value={settings.powerline.theme ?? "custom"}
                  onChange={(value) =>
                    updateSettings((current) => ({
                      ...current,
                      powerline: { ...current.powerline, theme: value ?? "custom" },
                    }))
                  }
                />
                <Switch
                  label={t("settings.statusline.minimalist")}
                  checked={settings.minimalistMode}
                  onChange={(event) =>
                    updateSettings((current) => ({
                      ...current,
                      minimalistMode: event.currentTarget.checked,
                    }))
                  }
                />
                <TextInput
                  label={t("settings.statusline.separator")}
                  value={settings.defaultSeparator ?? ""}
                  onChange={(event) =>
                    updateSettings((current) => ({
                      ...current,
                      defaultSeparator: event.currentTarget.value || undefined,
                    }))
                  }
                />
                <NumberInput
                  label={t("settings.statusline.compactThreshold")}
                  min={1}
                  max={99}
                  ...
                  onChange={(value) =>
                    updateSettings((current) => ({
                      ...current,
                      compactThreshold: Number(value) || 60,
                    }))
                  }
                />
              </Stack>
...
export function StatuslineSettingsPage({ searchValue = "" }: { searchValue?: string }) {
  ...
  const exportProfiles = async () => {
    const path = await saveDialog({
      defaultPath: "cli-manager-statusline-profiles-v1.json",
    ...
    try {
      ...
      toast.error(t("settings.statuslineProfiles.exportFailed"), {
        description: errorMessage(error),
      });
    } finally {
  ...
  const importProfiles = async () => {
    ...
    setTransferring(true);
    try {
      const analysis = await invoke<StatuslineImportAnalysis>(
        "statusline_profiles_analyze_import",
        { path, configDir: codexConfigDir ?? undefined },
      );
      ...
      for (const conflict of analysis.conflicts) {
        ...
        if (!action) return;
        if (action !== "overwrite" && action !== "skip" && action !== "rename")
          throw new Error("statusline_profiles_invalid_decision");
        if (conflict.active && action === "overwrite")
          throw new Error("statusline_profiles_active_overwrite_forbidden");
        const decision: StatuslineImportDecision = {
          tool: conflict.tool,
      ...
      await invoke("statusline_profiles_commit_import", {
        path,
      ...
      toast.success(
        t("settings.statuslineProfiles.imported")
          .replace("{claude}", String(analysis.claudeCount))
          .replace("{codex}", String(analysis.codexCount)),
      );
    } catch (error) {
      toast.error(t("settings.statuslineProfiles.importFailed"), {
        description: errorMessage(error),
      });
    } finally {
      setTransferring(false);
    ...
    <Stack gap="md">
      <Card className="border border-border bg-surface-container-low" radius="lg" p="md">
        <Group justify="space-between" mb="xs">
          <Text size="xs" c="var(--on-surface-variant)">
            {t("settings.statusline.chooseTool")}
          </Text>
          <Group gap="xs">
            <Button
              size="xs"
            ...
            >
              {t("settings.statuslineProfiles.importLibrary")}
            </Button>
            <Button
              size="xs"
              variant="light"
            ...
            >
              {t("settings.statuslineProfiles.exportLibrary")}
            </Button>
          </Group>
        </Group>
        <SegmentedControl
              ...
              label: (
                <Group gap="xs" justify="center">
                  ...
                  <Badge size="xs" variant="light" color="orange">
                    {t("settings.statusline.builtinRuntime")}
                  </Badge>
                </Group>
              ),
            },
              ...
              label: (
                <Group gap="xs" justify="center">
                  ...
                  <Badge size="xs" variant="light" color="green">
                    {t("settings.codexStatusline.native")}
                  </Badge>
                </Group>
              ),
            },
